...
SELECT * FROM author;
   -- 명시적 방법: 컬럼 목록 제공, 컬럼 목록의 숫자, 순서, 데이터 타입이 
...
UPDATE author
SET author_desc = '알쓸신잡 출연'; -- WHERE 절이 없으면 전체 레코드 변경(막혀있음)
UPDATE author
SET author_desc = '알쓸신잡_출연'
WHERE author_id = 2;
SELECT * FROM author;
...
ALTER TABLE author AUTO_INCREMENT = 3; -- 3부터 시작
-- 테이블 구조 확인
DESCRIBE author;
SELECT * FROM author;
INSERT INTO author (author_name) VALUES ('스티븐 킹');
INSERT INTO author (author_name, author_desc) VALUES ('류츠신', '삼체 작가');
INSERT INTO author VALUES (5,'다자이 오사무', '인간실격 작가');
INSERT INTO author (author_name) VALUES ('알베르 까뮈');
SELECT * FROM author;
...
INSERT INTO book (title, author_id) VALUES ('인간실격',5);
INSERT INTO book (title, author_id) VALUES ('쇼생크 탈출', 3);
INSERT INTO book (title, author_id) VALUES ('삼체', 4);
SELECT * FROM book;
ALTER TABLE book MODIFY pub_date DATE DEFAULT (CURDATE());
